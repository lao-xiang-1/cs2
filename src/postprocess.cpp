...
int* get_enemy_center(cv::Mat& img, float bbox[4]) {
    float l, r, t, b;
    float r_w = kInputW / (img.cols * 1.0);
    float r_h = kInputH / (img.rows * 1.0);
    ...
    int width = std::max(0, std::min(int(round(r - l)), img.cols - int(round(l))));
    int height = std::max(0, std::min(int(round(b - t)), img.rows - int(round(t))));
    int result[4] = { center_x, center_y, width, height };
    return result;
}

void get_enemy_info(std::vector<cv::Mat>& img_batch, std::vector<std::vector<Detection>>& res_batch, Enemy& enemy) {
    for (size_t i = 0; i < img_batch.size(); i++) {
        auto& res = res_batch[i];
        cv::Mat img = img_batch[i];
        int CT_count = 0, CT_head_count = 0, T_count = 0, T_head_count = 0;
        int L_CT_head_count = 0, L_T_head_count = 0;
        int S_CT_count = 0, S_CT_head_count = 0, S_T_count = 0, S_T_head_count = 0;
        for (size_t j = 0; j < res.size(); j++) {
            int* r = get_enemy_center(img, res[j].bbox);
            switch ((int)res[j].class_id) {
                case 0:  // CT
                    if (CT_count == 0) {
                        enemy.CT[0] = r[0] - 320;
                        enemy.CT[1] = r[1] - int(r[3] * 0.25) - 320;
                        enemy.CT[2] = r[2];
                        enemy.CT[3] = r[3];
                    }else if (abs(enemy.CT[0]) + abs(enemy.CT[1]) > abs(r[0] - 320) + abs(r[1] - int(r[3] * 0.25) - 320)) {
                        enemy.CT[0] = r[0] - 320;
                        enemy.CT[1] = r[1] - int(r[3] * 0.25) - 320;
                        enemy.CT[2] = r[2];
                        enemy.CT[3] = r[3];
                ...
                case 1:  // CT_head
                    if (CT_head_count == 0) {
                        enemy.CT_head[0] = r[0] - 320;
                        enemy.CT_head[1] = r[1] - 320;
                        enemy.CT_head[2] = r[2];
                        enemy.CT_head[3] = r[3];
                    }else if (abs(enemy.CT_head[0]) + abs(enemy.CT_head[1]) > abs(r[0] - 320) + abs(r[1] - 320)) {
                        enemy.CT_head[0] = r[0] - 320;
                        enemy.CT_head[1] = r[1] - 320;
                        enemy.CT_head[2] = r[2];
                        enemy.CT_head[3] = r[3];
                ...
                case 2:  // T
                    if (T_count == 0) {
                        enemy.T[0] = r[0] - 320;
                        enemy.T[1] = r[1] - int(r[3] * 0.25) - 320;
                        enemy.T[2] = r[2];
                        enemy.T[3] = r[3];
                    }else if (abs(enemy.T[0]) + abs(enemy.T[1]) > abs(r[0] - 320) + abs(r[1] - int(r[3] * 0.25) - 320)) {
                        enemy.T[0] = r[0] - 320;
                        enemy.T[1] = r[1] - int(r[3] * 0.25) - 320;
                        enemy.T[2] = r[2];
                        enemy.T[3] = r[3];
                ...
                case 3:  // T_head
                    if (T_head_count == 0) {
                        enemy.T_head[0] = r[0] - 320;
                        enemy.T_head[1] = r[1] - 320;
                        enemy.T_head[2] = r[2];
                        enemy.T_head[3] = r[3];
                    }else if (abs(enemy.T_head[0]) + abs(enemy.T_head[1]) > abs(r[0] - 320) + abs(r[1] - 320)) {
                        enemy.T_head[0] = r[0] - 320;
                        enemy.T_head[1] = r[1] - 320;
                        enemy.T_head[2] = r[2];
                        enemy.T_head[3] = r[3];
        ...
        enemy.CT_count = CT_count;
        enemy.CT_head_count = CT_head_count;
        enemy.T_count = T_count;
        enemy.T_head_count = T_head_count;
    }
}
